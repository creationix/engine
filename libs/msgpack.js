...
var flatten = bintools.flatten,
    ...
    uint8 = bintools.uint8,
    uint16 = bintools.uint16,
    uint32 = bintools.uint32,
    uint64 = bintools.uint64;
...
function decode(buf) {
  ...
  function readBin(len) {
    var buf = buffer.slice(offset, offset + len);
    offset += len;
    return buf;
  }
  ...
  function readFloat() {
    var num = new DataView(buffer).getFloat32(offset, false);
    offset += 4;
    return num;
  }

  function readDouble() {
    var num = new DataView(buffer).getFloat64(offset, false);
    offset += 8;
    return num;
  }
